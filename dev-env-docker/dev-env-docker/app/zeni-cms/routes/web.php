...
Route::middleware(['AccessLogin', 'CheckLogin'])->namespace('Admin')->prefix('admin')->group(function () {
    ...
    Route::group(['prefix' => 'cate'], function(){
        Route::get('/', 'CategoryController@index')->name('cate.getlistcate');
    ...
    Route::group(['prefix' => 'post'], function () {
        ...
        Route::get('/', 'PostController@index')->name('getlistView.posts');
        Route::post('/', 'PostController@index')->name('listView.posts');
        Route::get('addView', 'PostController@addView')->name('addView.posts');
        Route::post('adddata', 'PostController@addposts')->name('adddata.posts');
        Route::post('delPost', 'PostController@delData')->name('del.posts');
        Route::get('editView/{id}', 'PostController@editDataById')->name('editview.posts');
        Route::post('editData', ['as' =>'edit.posts', 'uses'=>'PostController@editData']);
        //===================//
        Route::get('/show-trash', 'PostController@showTrash')->name('tras.posts');
        Route::post('/show-trash', 'PostController@showTrash')->name('trash.posts');
        Route::get('restore-trash/{id}', 'PostController@restoreTrash')->name('restore.posts');
        Route::post('delete-trash', 'PostController@deleteTrash')->name('forcedelete.posts');
...
Route::post('ajax', function (Request $request) {
    ...
    $users = Users::where('name', 'like', '%' . $data . '%')->get();
